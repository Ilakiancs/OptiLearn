...
@router.get("/report")
async def teacher_report(week: str = Query(default=None)) -> Response:
    """
...
.banner{{background:#1a73e8;color:#fff;font-weight:700;font-size:12pt;padding:7pt 12pt;margin-bottom:18pt;-webkit-print-color-adjust:exact;print-color-adjust:exact;}}
...
.summary{{background:#f8f9fa;border-left:4px solid #1a73e8;padding:9pt 12pt;margin-bottom:14pt;font-size:10pt;direction:auto;unicode-bidi:plaintext;line-height:1.7;}}
...
{summary_html}
<div class="metrics">
  <div class="metric"><div class="metric-val" style="color:#1a73e8">{total}</div><div class="metric-lbl">Students</div></div>
...
@router.post("/chat")
async def teacher_chat(body: TeacherChatRequest) -> StreamingResponse:
...
async def create_schedule(body: ScheduledClassCreate) -> dict:
...
async def update_schedule(class_id: str, body: ScheduledClassUpdate) -> dict:
...
async def delete_schedule(class_id: str) -> dict:
...
async def update_settings(body: TeacherSettingsUpdate) -> dict:
